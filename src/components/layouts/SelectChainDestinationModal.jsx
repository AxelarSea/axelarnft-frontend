...
const SelectChainDestinationModal = (props) => {
    ...
    <Modal show={props.onShow} onHide={props.onHide}>
        ...
        <div className="modal-body space-y-20 pd-40">
            ...
            {data.map((item,index) => (
                <li 
                className={(props.myNftOn === item.topic) ? "hidden" : "list-group-item list-group-item-action justify-content-between"} 
                style={{border:'none', cursor:(props.myNftOn === item.topic) ? 'not-allowed' : 'pointer' , opacity:(props.myNftOn === item.topic) ? '0.6' : '1'}}   
                onClick={() => handleChangeChain(item.topic, item.chainId, item.img ,index)} 
                >
